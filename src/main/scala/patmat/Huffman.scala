...
object Huffman {
  ...
  /**
    ...
    * }
    */
  def times(chars: List[Char]): List[(Char, Int)] = chars.map(x => x -> 1).groupBy(_._1).map(y => (y._1, y._2.size)).toList
  ...
  /**
    ...
    */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortWith((x, y) => x._2 < y._2).map(t => Leaf(t._1, t._2))

  /**
    * Checks whether the list `trees` contains only one single code tree.
    */
  def singleton(ts: List[CodeTree]): Boolean = ts.length == 1
  /**
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case l :: r :: t => (Fork(l, r, chars (l) ::: chars(r), weight(l) + weight(r)) :: t).sortWith(weight(_) < weight(_))
    case _ => trees
  }
